	...
	{   		
		...
		{		  
		   if($user_directory_meta_flds_tmp[$inc]['field']) 
		   {
			   ...
			   $user_directory_meta_flds_tmp[$inc]['value'] =  dynamic_ud_format_meta_val($user_meta_fld, $dud_options, $fld_format); 	
			}
		} 
	   ...
	   {	
		...
		if($user_fullname !== '' && !($ud_hide_username === "hide"))
			$user_contact_info .= "\t<div class='dud_field_name'>" . $user_fullname . "</div>\n";
		...
		foreach ($sort_order_items as $item)
		{
			if($item === "Email")
			{
				if($user_directory_email && $user_email !== '') {
					$user_contact_info .= "\t\t\t<div class='dud_field_email'><a href=\"mailto:" . $user_email . "\" target=\"_top\">" . apply_filters('dud_set_user_email_display', $user_email, $user_id) . "</a></div>\n";	
					$line_cnt++;
				}
			}
			else if($item === "Website")
			{
				if($user_directory_website && $user_website !== '') {
					$user_contact_info .= "\t\t\t<div class='dud_field_website'><a href=\"" . $user_website . "\">" . $user_website . "</a></div>\n";	
					$line_cnt++;
				}
			}
			...
			{	
				...
				if(!empty($social_flds[0]) || !empty($social_flds[1]) || !empty($social_flds[2]) || !empty($social_flds[3]) || !empty($social_flds[4]) || !empty($social_flds[5]))
				{
					...
					$social_icons .= "\t\t\t<div class='dud_field_social' style='font-size:" . $ud_icon_size . "px !important;'>";
			...
			else
			{
				foreach ( $user_directory_meta_flds_tmp as $ud_mflds )
				{
					if(!empty($ud_mflds['key']) && $item === $ud_mflds['key'])			
					{
						...
						if((!empty($ud_mflds['value']) || $ud_mflds['value'] === "0") && !empty($ud_mflds['label'])) 
						{
							$css_line_cnt++;
							$user_contact_info .= "\t\t\t<div class='dud_line_" . $css_line_cnt . "'><span class='dud_label'><b>" . $ud_mflds['label'] . "</b></span>\n";
							$has_label = true;
						...
						//Print the field
						if((!empty($ud_mflds['value']) || $ud_mflds['value'] === "0"))
						{
							$dud_line = "";
							...
							{
								$css_line_cnt++;								
								$dud_line = "<div class='dud_line_" . $css_line_cnt . "'>";
							}
...
function dynamic_ud_format_meta_val($user_meta_fld, $dud_options, $format)
{
	...
	* 16-23 = Date Formatting
	* 24    = Multi-line Text Box
	* 30    = Hide MemberPress Hyphens
	* 31    = Phone Number (Australian Format)
	* 32    = Mobile Phone Link
	* 33    = Mobile Phone Link (Australian Format)
	* 34    = Image
	**********************************************/
	...
	if(!is_array($user_meta_fld) && (($format === "6") || ($format === "31") || ($format === "32") || ($format === "33")))
	{
		...
		else if($format === "32" || $format === "33")
		{
			...
			//echo "In the right if, phone is " . $phone . "<BR>";
			if(!empty($phone))
	...
	//MemberPress Hyphens
	else if(!is_array($user_meta_fld) && $format === "30")
	{
