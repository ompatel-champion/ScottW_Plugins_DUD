...
<form id="user_directory_options" action="options.php" method="post"">
...
<div class="dud-settings-section">
	<table class="form-table">
		<tr>
			<td colspan="3" style="line-height:22px;"><b>Instructions</b><br><hr>In the section below, fill out the criteria that must be met for a user to be *excluded* from the directory. To check the contents of the filter field for each user, turn Debug Mode to "on," then 
                                                      visit the directory while logged in as admin. The filter field data is listed under the heading "Exclude User Filter."<hr></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>Choose the state that the filter must be in for the user to be excluded.</td>
			<td></td>
		</tr>
		...
		<tr>
			...
			<td><input class="dd-menu-no-chk-box-width" type="text" id="ud_filter_fld_value" name="<?php echo $dud_option_name;?>[ud_filter_fld_value]" 
				value="<?php echo !empty($dud_options['ud_filter_fld_value']) ||  $dud_options['ud_filter_fld_value']==="0" ? esc_attr( $dud_options['ud_filter_fld_value'] ) : ""; ?>" maxlength="50"/></td>
			<td>Enter the *case sensitive* value that the filter field must be for the user to be excluded. This setting only applies if you selected Equal To, Not Equal To, Contains, or Does Not Contain in the dropdown above.</td>
			<td></td>
		</tr> 
		<tr>
	...
	<?php try { ?>
		<tr id="mp_hide_membership">
			<td><b>Show Users with the Following Subscription(s):</b></td>
			<td>
	...
	<tr>
		...
		<td>
			<select class="dd-menu-no-chk-box-width" name="<?php echo $dud_option_name;?>[ud_hide_username]" id="ud_hide_username">
					<OPTION value="show">Show Username</OPTION> 
					<OPTION value="hide" <?php echo (!empty($dud_options['ud_hide_username']) && $dud_options['ud_hide_username'] == "hide") ? "SELECTED" : ""; ?>>Hide Username</OPTION>  							  
			</select> 
		</td>
		<td>Optionally hide the user name shown by default at the beginning of each listing. IMPORTANT NOTE: All users must still have a Last Name or Display Name filled out in their WordPress profile.</td>
		<td></td>
	</tr>
				 ...
				 <tr id="meta_fld_<?php echo $inc; ?>">
					...
					<td><b>Format Meta Field <?php echo $inc; ?> As</b><br>
						<select name="<?php echo $dud_option_name;?>[dud_fld_format_<?php echo $inc; ?>]" id="dud_hyperlink_flds">
							...
							<OPTION value="34" <?php echo (!empty($dud_options['dud_fld_format_' . $inc]) && $dud_options['dud_fld_format_' . $inc] == "34") ? "SELECTED" : ""; ?>>Image (Should be a URL)</OPTION>
							...
							<OPTION value="30" <?php echo (!empty($dud_options['dud_fld_format_' . $inc]) && $dud_options['dud_fld_format_' . $inc] == "30") ? "SELECTED" : ""; ?>>Hide MemberPress Hyphens</OPTION>
	...
	<table class="form-table">
			...
			<tr>
				...
				<td>This hides the meta field selector dropdown on the front end and automatically looks for the search value within all the meta fields configured below. 
                    Please note: if you have an alpha links directory, the "Show Search Results" setting *must* be "Single Page Format."</td>
				<td></td>
			</tr>	
			<tr>
			...
			<table class="search-meta-flds">
				...
				<i>For a General Search, simply enter the meta key names of the fields to be searched in below. To allow searching on Last Name/Display Name or Email Address for a general search, select "show" in the dropdowns above. If not doing a General Search, you may also enter the Meta Key Name (required), Meta Field Label (required), Search Type (required), Search Dropdown Values (optional), and Search Dropdown Labels (optional) for each field below.<BR><BR><u><i>Search Type</u></i>: Specify whether the meta field should start with, contain, or exactly match the search value entered. 
...
function update_dynamic_srch_box_jquery( $input )
{
	...
	try
	{
		...
		else if(file_exists(plugin_dir_path(__FILE__) . 'dynamic-srch-js/jquery.dynamic-srch-box.js'))
			unlink(plugin_dir_path(__FILE__) . 'dynamic-srch-js/jquery.dynamic-srch-box.js');

		$newfile = fopen(plugin_dir_path(__FILE__) . 'dynamic-srch-js/jquery.dynamic-srch-box.js','w'); //create new file to write to with file number
...
/*function my_plugin_notice() {
    ...
    if ( !get_user_meta( $user_id, 'dud_exclude_user_addon_notice_dismissed' ) && current_user_can('manage_options') )
	{		
		$current_url = esc_url( home_url( '/' ) ) . 'wp-admin/options-general.php?page=user_directory&';
		echo '<div class="notice notice-warning"><p>Dynamic User Directory has a new <a href="http://sgcustomwebsolutions.com/wordpress-plugin-development/" target="_blank">Exclude User Filter Add-on</a> available now!&nbsp;&nbsp;<a href="' . $current_url . 'my-plugin-dismissed">Dismiss</a></p></div>';
	}
}
...
function my_plugin_notice_dismissed() {
    ...
    if ( isset( $_GET['my-plugin-dismissed'] ) )
        add_user_meta( $user_id, 'dud_exclude_user_addon_notice_dismissed', 'true', true );
}
